...
def projection_similarity(A, B):
    ...
    sim_dist = 1 - A.dot(B.T)
    idx_sort = np.argsort(sim_dist, axis=1)
    size = A.shape[0]
    partition_set = set()
    final_dist = 0
    for i in range(size):
        for item in idx_sort[i]:
            if item not in partition_set:
                partition_set.add(item)
                final_dist += sim_dist[i][item]
                break

    avg_dist = final_dist/size
    return avg_dist
...
def spectral_graph_coarsening(G,  n):
    ...
    for k in range(0, n):
        if e1[k] <= 1:
            ...
            #avg_project_dist = projection_similarity(Q.T, context)

            dist = measure.eig_partial_dist_k_two_end_n(e1, e2, ec, k)

            #dist += avg_project_dist*0.1
