...
class Hopfield(nn.Module):
    def __init__(self, config):
        ...
        self.context_weight = nn.Parameter(nn.init.normal_(torch.Tensor(self.dim, self.vocab_size), mean=0.0, std=0.1))
    ...
    def forward(self, context):
        ...
        for i in range(layer_num):
            ...
            if i != 0:
                ...
                value, idx = torch.topk(v1_plus, 10, dim=1)
                #print('before v1 plus')
                #print(value)
                #print(idx)
                #v1_plus = F.normalize(v1_plus)
                #value, idx = torch.topk(v1_plus, 10, dim=1)
                #print('after v1 plus')
                #print(value)
                #print(idx)
                ...
                #print(v1_plus)
                '''
                v1_plus_bak = torch.zeros_like(v1_plus).to(self.config['device'])

                value_2, idx_2 = torch.topk(v1_plus, 5, dim=1)
                v1_plus_bak[torch.arange(v1_plus_bak.shape[0]).unsqueeze(1), idx_2] = 1
                print('v2_value')
                print(value_2)
                print(idx_2)
                '''
            ...
            #print(v1)
            #print(v1)
            #v1 = F.relu(self.bn1(v1))
            #v1 = self.bn1(v1)
            #v1_final = v1_final*self.beta
