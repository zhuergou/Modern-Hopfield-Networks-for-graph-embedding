...
class ModelHandler(object):
    ...
    def __init__(self, config):
        ...
        '''
        length = sequence_index.shape[0]
        train_index = sequence_index[:int(length*7.0/8)]
        test_index = sequence_index[int(length*7.0/8):]

        self.trainset = DataIndex(train_index, self.graph, False)
        self.trainloader = DataLoader(self.trainset, batch_size=20, shuffle=True, num_workers=0)

        self.testset = DataIndex(test_index, self.graph, False)
        self.testloader = DataLoader(self.testset, batch_size=20, shuffle=True, num_workers=0)
        '''
    ...
    def train(self):
        ...
        while self._stop_condition(self._epoch):
            ...
            self.logger.write_to_file("\n>>> Dev Epoch: [{} / {}]".format(self._epoch, self.config['max_epochs']))
            self._run_epoch(self.testloader, self.adj, training=False)
    ...
    def _run_epoch(self, data_loader, adj, training=True):
        ...
        for i, (context, label, label_index) in enumerate(data_loader):
            ...
            '''            
            ##v2
            masked = masked.to(self.config['device'])
            masked_value = masked_value.to(self.config['device'])
            idx = idx.to(self.config['device'])

            masked_good = masked_good.to(self.config['device'])
            masked_good_value = masked_good_value.to(self.config['device'])
            idx_good = idx_good.to(self.config['device'])
            '''

            res = self.model.predict(context, label, label_index, adj, update=training, mode=mode)
            #res = self.model.predict_ver2(masked, masked_value, idx, masked_good, masked_good_value, idx_good, update=training, mode=mode)
    ...
    def _stop_condition(self, epoch, patience=15):
    ...
    def save_word2vec_format(self, fname, binary=False):
        ...
        if self.config['graph_dir'] == './cora.mat':
            print('YES YES YES YES')
            m = loadmat(self.config['graph_dir'])
            idx = m['idx'][0]
            vocab_len = len(idx)
            flag = True
        else:
        ...
        '''
        node_context = np.array([self.graph.binary[i] for i in range(self.graph.vocab_size)])

        node_context = node_context/(np.linalg.norm(node_context, axis=1, keepdims=True))

        memory = self.model.network.context_weight.cpu().detach().numpy()
        memory = memory/(np.linalg.norm(memory, axis=1, keepdims=True))
        vectors = node_context.dot(memory.T)
        ...
        with open(fname, 'w') as fout:
            ...
            for i in range(len(vocab)):
                ...
                '''
                ##topk version
                bar = row[np.argsort(row)[-self.config['top_value']]]
                row_binary = (row > bar).astype(float)
                row_final = row* row_binary
                row_final = row_final/np.linalg.norm(row_final)
                '''
...
class Graph(defaultdict):
    ...
    def get_one_hot_represent(self):
        ...
        for key, value in self.items():
            ...
            #self.node_embedding[key] = [0]*vocab_size
            #self.oneblock[key] = [0]*vocab_size

            ## for target
            self.target_label[key] = key
            self.target_onehot[key][key] = 1.0
            #self.node_embedding[key][key] = 1
            #self.oneblock[key][key] = 1.0

            ##note here
            #self.binary[key][key] = 1.0
            
            ## for neighbor
            for item in value:
                self.binary[key][item] = 1.0
                #self.oneblock[key][item] = 1.0

            #self.binary[key] = self.binary[key]/np.sqrt(np.sum(self.binary[key]))
            ##normalize the input data
            #if np.linalg.norm(self.binary[key]) > 0:
                #self.binary[key] = self.binary[key]/(np.linalg.norm(self.binary[key]))
                #self.binary[key] = self.binary[key]/(np.sum(self.binary[key]))
            #self.oneblock[key] = self.oneblock[key]/(np.linalg.norm(self.oneblock[key]))
            
            ## two hop neighbor

            ## degree vector
            #self.degree_vector[key] = 1.0/len(value)
            #self.degree_vector[key+vocab_size] = 1.0/len(value)
        

        '''
        total_neighbor = [value for key, value in self.binary.items()]
        print(len(total_neighbor))
        unique_neighbor = set(map(tuple, total_neighbor))
        print(len(unique_neighbor))
        '''
...
class DataIndex(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        '''
        masked = np.array(self.graph.oneblock[self.index[idx]])
        idx_mask = np.random.randint(self.graph.vocab_size)
        if masked[idx_mask] > 0:
            masked_value = 1
        else:
            masked_value = 0
        #masked_value = masked[idx_mask]
        masked[idx_mask] = 0
        masked = torch.from_numpy(masked)
        masked_value = torch.tensor(masked_value)
        idx_mask = torch.tensor(idx_mask)
        
        masked_good = np.array(self.graph.oneblock[self.index[idx]])
        idx_good = np.random.choice(np.nonzero(masked_good)[0])
        masked_good_value = 1
        #masked_good_value = masked_good[idx_good]
        masked_good[idx_good] = 0
        masked_good = torch.from_numpy(masked_good)
        masked_good_value = torch.tensor(masked_good_value)
        idx_good = torch.tensor(idx_good)
        '''
