...
class ModelHandler(object):
    ...
    def _run_epoch(self, data_loader, adj, training=True):
        ...
        for i, (context, label, label_index, masked, masked_value, idx, masked_good, masked_good_value, idx_good) in enumerate(data_loader):
        ...
        with open(fname, 'w') as fout:
            ...
            for i in range(len(vocab)):
                ...
                if np.linalg.norm(row) > 0:
                    row_final = row/np.linalg.norm(row)
                else:
                    row_final = row
